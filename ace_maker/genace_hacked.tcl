...
## 
## $Header: /devl/xcs/repo/env/Jobs/MDT/sw/Apps/debug/new_xmd/DataFiles/Attic/genace.tcl,v 1.1.2.13.12.2.16.1 2009/11/11 17:44:42 rajn Exp $
###############################################################################
...
proc impact_bit2svf { bitfile jtag_fpga_position jtag_devices } {
	...
	if { ![string match "*Programing completed successfully.*" $msg] } {
	    puts $msg
...
proc write_swsuffix { target_type elffile xmd_options saddr } {
    ...
    xcontinue $tgt
    # This is needed for dual MB systems and MB version 7.20.a (EDK 11)
...
proc parse_genace_optfile { optfilename } {
	...
	if { ![info exists cpu_target($i)] } {
	    puts "Target Type not defined for Processor $cpunr, Using default Target Type ppc_hw"
            # Rahul : hacked to support all MB system - Replaced "ppc_hw" with "mdm"
	    set cpu_target($i) "mdm"
	}
	incr i 1
    }
